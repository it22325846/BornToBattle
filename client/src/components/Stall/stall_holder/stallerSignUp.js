...
const SignUp = () => {
  ...
  useEffect(() => {
    ...
    if (existingUsername) {
      ...
      window.location.href = '/stallerprofile';
    }
  }, []);
  ...
  const handleSignUp = async () => {
    if (password === confirmPassword && !usernameExists) {
      try {
        ...
        localStorage.setItem('userType', "stall_owner");//set session for user type

        window.location.href = '/createStaller';
 ...
 <div style={{ maxWidth: '500px', maxHeight: '700px', width: '100%', overflow: 'auto', }}>
  <div className="card p-4">
    <h2 className="text-center mb-4" style={{ color: 'black' }}>Sign Up Page</h2>
    <form>
      <div className="mb-3">
